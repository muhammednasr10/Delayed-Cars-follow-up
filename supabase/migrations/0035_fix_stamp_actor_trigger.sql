-- Completes 0034 when run alone: schema + stamp trigger fix + family member sync.
...
where f.family_code = 't4'
  ...
  and v.parent_model_id = (
    select id from vehicle_models where upper(trim(name)) = 'T4' and model_kind = 'family' limit 1
  )
