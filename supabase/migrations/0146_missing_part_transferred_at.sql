-- Mark shortage lines closed via «ترحيل» (archived without truly resolving the shortage).
-- Badge «تم ترحيلها» should only appear for these lines / their vehicles.
...
-- Recreate detail view with transferred_at
...
-- Transfer closes the line without pretending install is complete; stamps transferred_at.
...
begin
  ...
  -- Close for workflow only — do not bump installed_qty (problem not resolved).
