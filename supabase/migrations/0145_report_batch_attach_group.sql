-- Allow attaching newly reported chassis/issues to an existing report_group_id
-- (fixes "add chassis" on multi-VIN edit creating a standalone row instead of joining the group).

drop function if exists public.report_missing_parts_batch(
  text[], uuid, jsonb, uuid, uuid, text, text, public.priority_level, text, text, uuid
);
...
begin
  ...
  -- Join an existing multi-chassis report, or create a group when reporting 2+ VINs.
