...
import type { BomItemCreateInput, BomItemDetail, BomItemUpdateInput } from '../Types/bom'
import type { BomIplLogisticsInput } from '../Utils/iplBomLogistics'
...
export const IPL_CORE_LOGISTICS_FIELDS = [
  'part_length',
  'part_width',
  'part_height',
  'part_volume',
...
export const IPL_OPTIONAL_LOGISTICS_FIELDS = [
  'carton_qty',
  'part_weight',
  'carton_weight',
  'rack_length',
...
export function pickLogisticsFields(
...
export function logisticsPayload(input: BomIplLogisticsInput): Record<string, string | null> {
...
export async function getBomItemsForPartIds(partIds: string[], modelName: string): Promise<Map<string, BomItemDetail>> {
  ...
  for (let i = 0; i < partIds.length; i += CHUNK) {
    ...
    for (const row of (data ?? []) as BomItemDetail[]) {
      if (!bomRowAppliesToModel(row, modelName)) continue
