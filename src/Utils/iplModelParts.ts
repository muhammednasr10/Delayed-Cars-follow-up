...
export function iplDisplayPartNumber(partNumber: string | null | undefined): string {
  ...
  if (!pn || isAutoGeneratedPartNumber(pn)) return ''
  return pn
}
...
export function mergePartToBomItem(part: Part, bom: BomItemDetail | undefined, modelName: string): BomItemDetail {
  ...
  if (bom && !isPendingBomItemId(bom.id)) {
    return {
      ...bom,
      part_number: iplDisplayPartNumber(bom.part_number) || iplDisplayPartNumber(part.part_number),
  ...
  // Pending: show the parts-list master on this model without inventing PN or qty.
  return {
    id: pendingBomItemId(part.id),
    part_id: part.id,
    vehicle_model_id: null,
    station_id: null,
    part_number: iplDisplayPartNumber(part.part_number),
