...
const DEPARTMENTS: ResponsibleDepartment[] = [
  'warehouse',
  'purchasing',
  'production',
  'quality',
...
export function TrainingSkillForm({ open, editing, stations, busy, onClose, onSubmit }: Props) {
    ...
    setForm(
      editing
        ? {
            ...
            skillNameAr: editing.skillNameAr ?? '',
            skillNameEn: editing.skillNameEn ?? '',
            description: editing.description ?? '',
            department: editing.department ?? '',
            stationId: editing.stationId ?? '',
            validityDays: editing.validityDays != null ? String(editing.validityDays) : '',
            ...
            requiredManpowerCount: String(editing.requiredManpowerCount ?? 1),
  ...
  async function submit() {
    ...
    const ok = await onSubmit({
      ...
      skillNameAr: form.skillNameAr || null,
      skillNameEn: form.skillNameEn || null,
  ...
  return (
    <Modal
      open={open}
      title={editing ? t('training.skill.editTitle') : t('training.skill.addTitle')}
      ...
      footer={
        <>
          ...
          >
            {busy ? t('common.saving') : editing ? t('common.saveEdit') : t('common.save')}
          </button>
        </>
      }
    >
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Field label={t('training.skill.code')} required error={errors.skillCode}>
          <input
            className={inputCls(errors.skillCode)}
        ...
        <Field label={t('training.skill.nameAr')} required error={errors.skillNameAr}>
          <input
            className={inputCls(errors.skillNameAr)}
            value={form.skillNameAr}
            onChange={e => set('skillNameAr', e.target.value)}
          />
        </Field>
        <Field label={t('training.skill.nameEn')}>
          <input
            className={inputCls()}
            value={form.skillNameEn}
            onChange={e => set('skillNameEn', e.target.value)}
            dir="ltr"
          />
        </Field>
        ...
        <Field label={t('training.skill.standardTime')}>
          <input
            ...
            step="0.1"
            className={inputCls()}
            value={form.standardTimeMinutes}
            onChange={e => set('standardTimeMinutes', e.target.value)}
          />
        </Field>
        <Field label={t('training.skill.manpower')}>
          <input
            ...
            min={1}
            className={inputCls()}
            value={form.requiredManpowerCount}
            onChange={e => set('requiredManpowerCount', e.target.value)}
          />
        </Field>
        ...
        <Field label={t('training.skill.critical')}>
          <ActiveToggle
            active={form.isCritical}
            activeLabel={t('common.yes')}
            inactiveLabel={t('common.no')}
            onChange={v => set('isCritical', v)}
          />
        </Field>
        <Field label={t('training.skill.status')}>
          <ActiveToggle
            active={form.isActive}
            activeLabel={t('org.f.active')}
            inactiveLabel={t('org.f.inactive')}
