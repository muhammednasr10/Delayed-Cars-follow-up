...
export function EmployeeTrainingRecordForm({
  open,
  editing,
  preset,
  employees,
    ...
    setForm(
      editing
        ? {
            ...
            lastEvaluationDate: editing.lastEvaluationDate ?? '',
  ...
  async function submit() {
    ...
    const ok = await onSubmit({
      ...
      lastEvaluationDate: form.lastEvaluationDate || null,
  ...
  return (
    <Modal
      open={open}
      title={editing ? t('training.rec.editTitle') : t('training.rec.addTitle')}
    ...
    >
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <Field label={t('training.rec.skill')} required error={errors.skillId}>
          <select
            className={inputCls(errors.skillId)}
            value={form.skillId}
            ...
            {skills
              ...
              .map(s => (
                <option key={s.id} value={s.id}>
                  {s.skillCode} - {s.skillNameAr || s.skillNameEn}
                </option>
              ))}
          </select>
        </Field>
        <Field label={t('training.rec.level')}>
          <select
            className={inputCls()}
            value={form.level}
            onChange={e => set('level', e.target.value as TrainingLevel)}
          >
        ...
        <Field label={t('training.rec.lastEval')}>
          <input
            ...
            onChange={e => set('lastEvaluationDate', e.target.value)}
          />
        </Field>
