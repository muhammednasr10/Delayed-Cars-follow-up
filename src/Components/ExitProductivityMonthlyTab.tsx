...
export function ExitProductivityMonthlyTab() {
  ...
  function syncFromDaily() {
    const tallies = tallyDeliveredByModelDay(vehicles, year, month)
    setGrid(prev => {
      const next = new Map(prev)
  ...
  async function save() {
    ...
    setSuccess('')
    try {
      await bulkUpsertExitProductivity(exitGridToInputs(models, year, month, grid))
  ...
  function renderColumnValue(col: ModelColumn, workDate: string) {
    if (col.kind === 'family') {
      ...
      return <span className="font-black text-emerald-300">{value || '—'}</span>
    }
              ...
              {columns.map(col => (
                <th
                  ...
                  className={`${cell} min-w-[56px] text-xs font-black ${
                    col.kind === 'family' ? 'bg-emerald-500/10 text-emerald-200' : 'text-slate-400'
                  }`}
                >
                  {col.label}
                  {col.kind === 'family' && (
                    <span className="mt-0.5 block text-[9px] font-bold text-emerald-300/80">
                      {t('productivity.exitMonthly.familyTotal')}
                    </span>
                  )}
                </th>
              ))}
                ...
                <tr key={workDate} className="bg-slate-900/30 hover:bg-slate-800/40">
                  ...
                  {columns.map(col => (
                    <td
                      ...
                      className={`${cell} ${col.kind === 'family' ? 'bg-emerald-500/5' : ''}`}
                    >
            ...
            <tr className="bg-slate-950/80 font-black">
              ...
              {columns.map(col => (
                <td
                  ...
                  className={`${cell} ${col.kind === 'family' ? 'text-emerald-300' : 'text-cyan-300'}`}
                >
