...
export function EmployeeTrainingMatrixTab({ employees, skills, records, canManage, onChanged, notify }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter(r => {
      if (term && !r.employeeName.toLowerCase().includes(term) && !r.employeeCode.toLowerCase().includes(term))
        return false
  ...
  function openEdit(r: EmployeeTraining) {
    setEditing(r)
    setPreset(null)
    setFormOpen(true)
  }
  function openCell(emp: Employee, skill: TrainingSkill, rec: EmployeeTraining | null) {
    if (rec) openEdit(rec)
      ...
      setEditing(null)
      setPreset({ employeeId: emp.id, skillId: skill.id })
      setFormOpen(true)
    }
  }
  ...
  async function suspend(r: EmployeeTraining) {
    setBusy(true)
    try {
      await setTrainingRecordActive(r.id, false)
      await onChanged()
      ...
      <div className="card-industrial p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex rounded-xl bg-slate-800 p-1">
            ...
            >
              <Table2 className="h-4 w-4" /> {t('training.view.table')}
            </button>
            <button
              onClick={() => setView('grid')}
              className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-bold ${view === 'grid' ? 'bg-cyan-500 text-slate-950' : 'text-slate-300'}`}
            >
              <LayoutGrid className="h-4 w-4" /> {t('training.view.grid')}
            </button>
          </div>
          {canManage && (
            <button
            ...
            >
              <Plus className="mr-1 inline h-4 w-4" /> {t('training.addRecord')}
            </button>
          )}
        </div>
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <select
            ...
            onChange={e => setFilters(p => ({ ...p, skillId: e.target.value }))}
          >
          ...
          <select
            className="input-dark"
            value={filters.level}
            onChange={e => setFilters(p => ({ ...p, level: e.target.value as TrainingLevel | '' }))}
          >
          ...
          <select
            ...
            onChange={e => setFilters(p => ({ ...p, status: e.target.value as TrainingStatus | '' }))}
          >
          ...
          <select
            ...
            onChange={e => setFilters(p => ({ ...p, expiry: e.target.value as Filters['expiry'] }))}
          >
              ...
              <tbody className="divide-y divide-slate-800">
                {filtered.map(r => (
                  <tr key={r.id} className="bg-slate-900/30 hover:bg-slate-800/40">
                    ...
                    <td className="table-cell">
                      <TrainingLevelBadge level={r.level} />
                    </td>
                    <td className="table-cell">
                    ...
                    <td
                      className={`table-cell ${r.isExpired ? 'text-orange-300' : r.isNearExpiry ? 'text-yellow-300' : 'text-slate-400'}`}
                      dir="ltr"
                    >
                      {r.expiryDate ?? '-'}
                    </td>
