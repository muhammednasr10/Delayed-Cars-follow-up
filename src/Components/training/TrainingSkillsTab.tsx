...
export function TrainingSkillsTab({ skills, stations, canManage, onChanged, notify }: Props) {
  ...
  async function toggle(s: TrainingSkill) {
    setBusy(true)
    try {
      await setSkillActive(s.id, !s.isActive)
      await onChanged()
    } catch (e) {
              ...
              {skills.map(s => (
                <tr key={s.id} className="bg-slate-900/30 hover:bg-slate-800/40">
                  ...
                  <td className="table-cell font-bold text-slate-100">
                    ...
                    {s.isCritical && (
                      <span className="ml-2 inline-flex rounded-full bg-red-500/15 px-2 py-0.5 text-[10px] font-bold text-red-300 ring-1 ring-red-400/30">
                        {t('training.skill.critical')}
                      </span>
                    )}
                  </td>
                  <td className="table-cell text-slate-300">{s.department ? t(`department.${s.department}`) : '-'}</td>
                  <td className="table-cell text-slate-300">
                    {stations.find(st => st.id === s.stationId)?.station_number ?? '-'}
                  </td>
                  ...
                  {canManage && (
                    <td className="table-cell">
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => toggle(s)}
                          className={`rounded-lg p-2 ${s.isActive ? 'bg-red-500/15 text-red-200' : 'bg-emerald-500/15 text-emerald-200'}`}
                        >
