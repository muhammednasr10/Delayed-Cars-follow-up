  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
        <StatCard
          title={t('training.dash.totalEmp')}
          value={stats.total}
          tone="cyan"
          icon={<Users className="h-6 w-6" />}
        />
        <StatCard
          title={t('training.dash.qualified')}
          value={stats.qualified}
          tone="cyan"
          icon={<CheckCircle2 className="h-6 w-6" />}
        />
        <StatCard
          title={t('training.dash.inTraining')}
          value={stats.inTraining}
          tone="orange"
          icon={<GraduationCap className="h-6 w-6" />}
        />
        <StatCard
          title={t('training.dash.expired')}
          value={stats.expired}
          tone="red"
          icon={<AlertTriangle className="h-6 w-6" />}
        />
        <StatCard
          title={t('training.dash.near')}
          value={stats.near}
          tone="orange"
...
function ExpiryList({ title, rows }: { title: string; rows: EmployeeTraining[] }) {
  ...
  return (
    <div className="card-industrial overflow-hidden">
      <div className="border-b border-slate-800 p-4 font-black text-white">
        {title} ({rows.length})
      </div>
