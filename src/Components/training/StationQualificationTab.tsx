...
export function StationQualificationTab({ employees, required, records, stations }: Props) {
  ...
  const stationRequired = useMemo(
    () => required.filter(r => r.isActive && r.stationId === stationId),
    [required, stationId]
  )
        ...
        <>
            ...
            <div className="mb-3 flex flex-wrap items-center gap-3">
              ...
              {station?.line_name && (
                <span className="text-sm text-slate-400">
                  {t('station.line')}: {station.line_name}
                </span>
              )}
              {station?.work_areas?.name && (
                <span className="text-sm text-slate-400">
                  {t('station.area')}: {station.work_areas.name}
                </span>
              )}
            </div>
            ...
            {stationRequired.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {stationRequired.map(r => (
                  <span
                    key={r.id}
                    className="inline-flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-900/60 px-3 py-1.5 text-sm"
                  >
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div className="card-industrial overflow-hidden">
              <div className="flex items-center gap-2 border-b border-slate-800 p-4 text-emerald-300">
                <CheckCircle2 className="h-5 w-5" />
                <span className="font-black">
                  {t('training.qual.qualified')} ({qualified.length})
                </span>
              </div>
              <div className="divide-y divide-slate-800">
                {mandatoryCount === 0 && <div className="p-4 text-sm text-slate-400">{t('training.qual.noReq')}</div>}
                {mandatoryCount > 0 && qualified.length === 0 && (
                  <div className="p-4 text-sm text-slate-400">{t('training.qual.none')}</div>
                )}
                {qualified.map(q => (
            ...
            <div className="card-industrial overflow-hidden">
              <div className="flex items-center gap-2 border-b border-slate-800 p-4 text-red-300">
                <XCircle className="h-5 w-5" />
                <span className="font-black">
                  {t('training.qual.notQualified')} ({notQualified.length})
                </span>
              </div>
              <div className="divide-y divide-slate-800">
                {notQualified.map(q => (
