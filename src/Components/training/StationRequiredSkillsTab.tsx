...
import {
  createStationRequiredSkill,
  deleteStationRequiredSkill,
  updateStationRequiredSkill
...
export function StationRequiredSkillsTab({ required, stations, skills, canManage, onChanged, notify }: Props) {
  ...
  async function confirmDelete() {
    ...
    setBusy(true)
    try {
      await deleteStationRequiredSkill(deleteTarget.id)
      await onChanged()
  ...
  return (
    <div className="card-industrial overflow-hidden">
            ...
            <thead className="bg-slate-950/90">
              <tr>
                <th className="table-cell text-xs font-black uppercase text-slate-400">
                  {t('training.srsCols.stationCode')}
                </th>
                <th className="table-cell text-xs font-black uppercase text-slate-400">
                  {t('training.srsCols.stationName')}
                </th>
      ...
      <ConfirmDialog
        ...
        message={deleteTarget ? `${stationLabel(deleteTarget.stationId)} — ${deleteTarget.skillName}` : ''}
