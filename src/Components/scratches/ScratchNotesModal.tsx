...
import { MessageSquare, ScanLine, Send, Trash2, User } from 'lucide-react'
...
import {
  ...
  deleteScratchNote,
  getScratchNotes
} from '../../services/scratchNotesService'
import type { ScratchNote, ScratchNoteTarget } from '../../Types/scratch'

type Props = {
  target: ScratchNoteTarget | null
  onClose: () => void
}

function authorLabel(note: ScratchNote): string {
...
export function ScratchNotesModal({ target, onClose }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      setNotes(await getScratchNotes(target.scratchId))
    } catch (err) {
  ...
  async function removeAll() {
    ...
    setError('')
    try {
      await clearScratchNotes(target.scratchId)
      setNotes([])
    } catch (err) {
      const msg = err instanceof Error ? err.message : t('common.error')
      setError(msg.includes('admin') || msg.includes('Permission') ? t('mp.thread.deleteDenied') : msg)
  ...
  async function submit(e: FormEvent) {
    ...
    setError('')
    try {
      const note = await addScratchNote(target.scratchId, draft)
